...
Input Layer (IL) - (28 x 28 x 1) image

Convolutional Layer 1 (CL1):
    - Input: (28 x 28 x 1) 
    - Initially, pad borders with 2 px wide of 0s
    - 32 (5 x 5 x 1) filters w/ ReLU activation
    - Output: (28 x 28 x 32) 

Pooling Layer 1 (PL1):
    - Input: (28 x 28 x 32) 
    - (2 x 2) filter w/ stride of 2
    - Output: (14 x 14 x 32)

Convolutional Layer 2 (CL2):
    - Input: (14 x 14 x 32)
    - Initally, pad borders with 2 px wide of 0s
    - 64 (5 x 5 x 32) filters w/ ReLU activation
    - Output: (14 x 14 x 64)
...
Fully Connected Layer 1 (FCL1)
    - Input: (7 x 7 x 64)
    - 1024 neurons, so for each pixel there are 1024 weights
    - Output (1024) 
...
Ideas: I think I will still batch learn, but would store the inputs to the FCL1 in an intermediate between PL2 and FCL1, 
until it accumulates up to 100 images within FCL1.
...
class ConvolutionLayer:
    def __init__(self, n_filters, filter_height_width, n_incoming_depth, next_layer = None, previous_layer = None) -> None:
        # Connect the layers
        self.next_layer = next_layer
        ...
        self.n_incoming_depth = n_incoming_depth
        self.forward_pass_data = None

        # Initialize filters, bias, and dropout (forward prop)
        self.weights = np.random.rand(n_filters, filter_height_width, filter_height_width) - .5
        self.bias = np.random.rand(n_filters, filter_height_width, filter_height_width) - .5
        # self.dropout = None  # ! Decided not to add dropout, but can be next steps after implementing everything

    def pad_border(self, input):
        return np.pad(input, 2, 'constant', constant_values=0)

    def forward_propagation(self, forward_pass_data):

        self.forward_pass_data = forward_pass_data

        padded_input = pad_border(forward_pass_data)
...
class FullyConnectedLayer:
    ...
    def ReLU_activation_function(self, output) -> (np.ndarray):
        """ Passes the input through ReLU activation function """

        return np.maximum(0, output)


    def ReLU_gradient(self, upstream_gradient) -> (np.ndarray):
