...
# batch_size - the number of training images before NN parameters are updated
batch_size = 100
...
class InputLayer:
    ...
    def generate_training_data(self) -> None:
        ...
        train_data = self.train_data[self.train_data_index:
                                     self.train_data_index + batch_size].T
        train_images, train_labels = train_data[1:], train_data[0].astype(
            np.int32)
...
class FullyConnectedLayer:
    def __init__(self, n_neurons, n_incoming_neurons, is_output_layer=False, next_layer=None, previous_layer=None) -> None:
        ...
        self.weights = np.random.rand(
            n_neurons, n_incoming_neurons) - .5  # (n, 784) or (10. n)
        self.bias = np.random.rand(n_neurons, 1) - .5
        self.dropout = None

    def generate_dropout(self):
        """ Randomly generates the dropout of this layer"""
        def droupoutRateLambda(x): return 1/0.6 if x < 0.6 else 0
        self.dropout = np.array(list(map(droupoutRateLambda, np.random.rand(
            self.n_neurons)))).reshape((self.n_neurons, 1))
    ...
    def back_propagation(self, upstream_gradient) -> None:
        ...
        if self.is_output_layer == False:
            upstream_gradient = self.dropout * upstream_gradient
        ...
        if self.is_output_layer == False:
            downstream_gradient *= self.ReLU_gradient(self.forward_pass_data)

        # Weights derivate is dL/dw = dL/df * df/dw = upstream gradient * forward pass data, since df/dw = forward pass data
        avg_weights_deriv = (upstream_gradient @
                             self.forward_pass_data.T) / batch_size

        # Bias derivate is dL/db = dL/df * df/db = upstream_gradient, since df/db = 1 (add gate)
        avg_bias_deriv = np.sum(upstream_gradient, axis=1).reshape(
            self.n_neurons, 1) / batch_size
...
class SoftMaxLayer:
    ...
    def forward_propagation(self, forward_pass_data, is_training) -> None:
        ...
        self.softmax = np.exp(forward_pass_data) / \
            sum(np.exp(forward_pass_data))
    ...
    def back_propagation(self) -> None:
        ...
        self.softmax[train_labels, np.arange(
            batch_size)] = self.softmax[train_labels, np.arange(batch_size)] - 1
...
def print_current_metrics(epoch) -> None:
    ...
    print("Some stats:")
    print(
        f"avg_time_FCL1_forward_prop: {accum_time_FCL1_forward_prop/(epoch * 60000)}")
    print(
        f"avg_time_FCL2_forward_prop: {accum_time_FCL2_forward_prop/(epoch * 60000)}")
    print(
        f"avg_time_FCL1_backward_prop: {accum_time_FCL1_backward_prop/(epoch * 60000)}")
    print(
        f"avg_time_FCL2_backward_prop: {accum_time_FCL2_backward_prop/(epoch * 60000)}")
    print(
        f"avg_time_SML_forward_prop: {accum_time_SML_forward_prop/(epoch * 60000)}")
    print(
        f"avg_time_SML_backward_prop: {accum_time_SML_backward_prop/(epoch * 60000)}")
    print(
        f"avg_time_CEL_forward_prop: {accum_time_CEL_forward_prop/(epoch * 60000)}")
    print()
...
def generate_random_test(train_data, num_tests):
    ...
    test_images, test_labels = permutated_train_data[1:], permutated_train_data[0].astype(
        np.int32)
...
def train_neural_network(train_data, n_neurons, epochs, batch_size, learning_rate=0.001) -> None:
    ...
    list_layers = [input_layer, first_fully_connected_layer,
                   output_layer, softmax_layer, cross_entropy_layer]
    ...
    for epoch in range(epochs):
        ...
        for c_round in range(600):
            ...
            if c_round % 100 == 0:
                validation_images, validation_labels = generate_random_test(
                    train_data, 10000)
                prediction = input_layer.forward_propagation(
                    validation_images, False)
                n_corrects = np.sum(prediction == validation_labels)
                print(
                    f"Epoch {epoch}: Round {c_round:5d}: accuracy={(n_corrects/10000.0):0.6f} with {n_corrects} corrects")
...
def main() -> None:
    ...
    train_data = np.append(train_labels.reshape(
        1, len(train_labels)), train_images.T, axis=0).T  # each row is a training data
    ...
    print(
        f"Finished reading all the images into the program, in {end - start} seconds")
    ...
    neural_network = train_neural_network(
        train_data, n_neurons, epochs, batch_size)
